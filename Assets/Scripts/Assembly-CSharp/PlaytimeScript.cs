...
public class PlaytimeScript : MonoBehaviour
{
    ...
    {
        if (other.transform.name == "Player" & !this.ps.jumpRope & this.ps.playtime.playCool <= 0f)
        {
            this.ps.jumpRopeScreen.SetActive(true);
            this.ps.jumpRope = true;
            this.ps.frozenPosition = base.transform.position;
			this.agent.speed = 0f;
			this.ps.Frozen = true;
        }
    }
